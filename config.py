...
df_collab = pd.read_csv('df_collab.csv')
df_product=pd.read_csv('df_product.csv')
df_review_full = pd.read_csv('df_review_full.csv')  
...
top_products = df_review_full['product_id'].value_counts().reset_index()
...
top_products = pd.concat([top_products, df_product[['product_id', 'product_name', 'image','price']]], axis=1)
