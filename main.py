...
def run_recommender_app_collab():
    ...
    if st.button("Find Another Customer"):
        ...
        # Suggest 10 random customer names
        random_customer_names = random.sample(df_collab['customer_name'].tolist(), 10)
        st.write("Randomly suggested customer names:")
...
if __name__ == "__main__":
    ...
    if choice == 'Business Objective':
        ...
        st.dataframe(df[["product_id", "product_name","price","rating","description","brand","group"]].head(3))
        st.write("###### Data before cleaning includes the combination of name, description, brand, and group")
        st.dataframe(df[["product_id", "description"]].head(3))
        ...
        st.dataframe(df_review_full.head(3))
